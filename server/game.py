...
class Board(CardList):
  def getPrivate(self):
    return [{"pid": card.pid, "hid": card.hid, "left": str(-420 + 210 * (i % 4)) + "px", "bottom": str(200 + 300 * i / 4) + "px"} for i, card in enumerate(self.cards)]
...
class Player:
  ...
  def info(self):
    return {"name": self.user.name(), "character": {"hid": self.character}, "life": self.life, "maxlife": self.maxlife,
      "hand": self.hand.getPublic(), "board": self.board.getPrivate(), "role": self.role if self.role == "sheriff" or self.life == 0 else "unknown"}

  def get_info(self, _for):
    if _for == self.user:
      return {"name": self.user.name(), "character": {"hid": self.character}, "life": self.life, "maxlife": self.maxlife,
        "hand": self.hand.getPrivate(), "board": self.board.getPrivate(), "role": self.role}
    else:
      return {"name": self.user.name(), "character": {"hid": self.character}, "life": self.life, "maxlife": self.maxlife,
        "hand": self.hand.getPublic(), "board": self.board.getPrivate(), "role": self.role if self.role == "sheriff" or self.life == 0 else "unknown"}
...
class Match:
  ...
  async def addUser(self, _user):
    ...
    await self.__broadcast({"event": "match_edit", "match": self.info()}, _user)
    
    if self.status == 'ready':
      self.players[_user.name()].user = _user
  ...
  async def aGetMatchStartData(self, _user, _message):
    await _user.send({"turns": self.turns, "turn": self.turnPlayer.user.name(), "role": self.players[_user.name()].role, "players": [player.get_info(_user) for player in self.players.values()],
      "discards": self.discards.getPrivate(), "revealed": self.revealed.getPrivate()}, _message, 200)

  async def aCardMove(self, _user, _message):
    owner = _message["from"].split('/')

    #if "hid" not in _message:
    #  await _user.send({}, _message, 500)
  ...
  def isOpen(self, _user):
    return (self.status == 'open' and len(self.users) < self.MAX_PLAYERS) or (self.status == 'ready' and _user.name() in self.players)
